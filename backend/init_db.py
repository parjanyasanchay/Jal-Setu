"""
Database Initialization and Migration Helper for Jal Setu.

Creates tables, enables PostGIS if supported by PostgreSQL,
and seeds initial national watershed benchmarks and the demo officer user if empty.
"""
...
from database import Base, engine, SessionLocal, init_postgis
...
def init_database():
    """Initializes schema and seeds default data if tables are empty."""
    init_postgis(engine)
    Base.metadata.create_all(bind=engine)

    db = SessionLocal()
    try:
        # 1. Seed demo officer user if missing
        officer = db.query(User).filter(User.email == "officer@jalsetu.gov.in").first()
        if not officer:
            demo_user = User(
                user_id="USR-" + uuid.uuid4().hex[:8].upper(),
                full_name="Pari Jadhav",
                email="officer@jalsetu.gov.in",
                password_hash=hash_password("JalSetu@2026"),
            ...
            db.add(demo_user)
            db.commit()
            print("[INIT_DB] Default demo officer account initialized.")

        # 2. Seed national benchmark watersheds if table is empty
        count = db.query(Watershed).count()
        if count == 0:
            ...
            db.commit()
            print(f"[INIT_DB] Successfully seeded {len(national_watersheds)} benchmark watersheds.")
    finally:
        db.close()


if __name__ == "__main__":
    print("[INIT_DB] Initializing Jal Setu database...")
    init_database()
